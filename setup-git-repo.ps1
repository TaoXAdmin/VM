# Script PowerShell d'initialisation du dépôt Git pour le projet de site web d'illusionniste contemporain
...
if (Test-Path "package.json") {
    # Si package.json existe, il faudrait idéalement utiliser un outil pour modifier le JSON
    # Mais pour simplifier, nous allons créer un nouveau package.json temporaire
    Write-Host "⚠️ package.json existe déjà. Veuillez ajouter manuellement le script 'prepare': 'husky install' à package.json" -ForegroundColor Yellow
} else {
    Write-Host "📝 Création d'un package.json de base..." -ForegroundColor Yellow
    @'
{
